...
describe("Gameboard", () => {
  ...
  test("places new destroyer on grid", () => {
    board.placeShip(0, 2, "destroyer", "horizontal");

    for (let i = 0; i < 10; i++) {
      for (let j = 0; j < 10; j++) {
        if (j === 2 && (i === 0 || i === 1)) {
          expect(board.grid[i][j]).toEqual({
            hit: false,
  ...
  test("places new submarine on grid", () => {
    board.placeShip(0, 2, "submarine", "horizontal");
  ...
  test("returns false for ship originating outside grid bounds", () => {
    expect(board.placeShip(0, 11, "destroyer", "horizontal")).toBeFalsy();
  });

  test("returns false for vertical ship whose length goes out of bounds", () => {
    expect(board.placeShip(0, 9, "destroyer", "vertical")).toBeFalsy();
  });

  test("places a ship vertically", () => {
    board.placeShip(0, 8, "destroyer", "vertical");
  ...
  test("returns false for horizontal ship whose length goes out of bounds", () => {
    expect(board.placeShip(9, 0, "destroyer", "horizontal")).toBeFalsy();
  });

  test("returns false for placing ship on top of other ship", () => {
    board.placeShip(0, 0, "submarine", "horizontal");

    expect(board.placeShip(1, 0, "destroyer", "horizontal")).toBeFalsy();
    expect(board.placeShip(1, 0, "destroyer", "vertical")).toBeFalsy();
  });

  test("records hit on ship", () => {
    board.placeShip(0, 1, "destroyer", "horizontal");
  ...
  test("Sinks ship if all coordinates hit", () => {
    ...
    expect(board.grid[1][0].ship.sunk).toBe(false);

    board.receiveAttack(1, 0);

    expect(board.grid[0][0].ship.sunk).toBe(true);
    expect(board.grid[1][0].ship.sunk).toBe(true);
  });
});
